...
> 当你手滑把 `ls` 敲成 `sl`、把 `git` 敲成 `got` 时，它会调用大模型 API，以规定的人设输出回复，并给出正确的命令建议。
...
- 🗣️ **人设可配置**：系统提示词全部在 `config.env` 里维护，内置 `sarcastic` / `angry` / `tsundere`，可自由新增风格
- ⚡ **同步执行**：嘲讽直接出现在提示符之前
- ⏳ **等待转圈**：调用 AI 时显示转圈动画（文案可配置），非 UTF-8 终端自动回退 ASCII
- 🚫 **零依赖**：`mock.py` 仅用 Python 3 标准库（`urllib.request`）
- 🔒 **永不报错**：请求超时（默认 6 秒）/ 断网 / 未配置 Key 时，静默回退到本地的语录
...
[同步调用 mock.py（嘲讽先于提示符渲染）]
        ...
        └── 3. 读取 config.env 中配置的 System Prompt 人设
        │
        ▼
[彩色渲染：红色标头 + 回复 + 青色命令建议]
```
...
vim config.env
```
具体配置项说明请参考 `config.env.example` 文件里的注释。
...
**Q：不填 API Key 会怎样？**
完全正常使用——`mock.py` 会静默走本地语录兜底，只是内容固定、没有"个性化嘲讽"。
